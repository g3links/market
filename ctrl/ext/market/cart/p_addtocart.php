...
if ($idcart !== false) {
    ...
    $result->lbl_title = \model\lexi::get('ext/market', 'sys024');
    $result->lbl_qty = \model\lexi::get('ext/market', 'sys007');
} else {
    $result = new stdClass();
    $result->msgerror = \model\lexi::get('ext/market', 'sys025');
}

echo json_encode($result);
//echo \model\utils::format('<div>{0} {1} - {2}, {3}: {4}</div>', \model\lexi::get('ext/market', 'sys024'), $result->idproduct, $result->name, \model\lexi::get('ext/market', 'sys007'), $result->qty);
